...
class BattlefieldEntity(Base):
    ...
    max_power   = Column(Integer ,default= 22)
    #to correct
...
class VesselEntity(Base) :
    ...
    uselist=False, cascade="all, delete-orphan")
    type        = Column(String,default="Cruiser,Destroyer,Frigate,Submarine")

''''class CruiserEntity(Base) :
    __tablename__ = "cruiser"
    x           = Column(Integer,ForeignKey("vessel.x"))
    y           = Column(Integer,ForeignKey("vessel.y"))
    z           = Column(Integer,ForeignKey("vessel.z"))'''
...
class WeaponEntity(Base) :
    ...
    type        = Column(String,default ="AirMissileLauncher,SurfaceMissileLauncher,TorpedoLauncher")
